...
-- In-memory lockout table for guild alerts (keyed by guild name, not persisted).
local guild_alert_lockout = {}
...
function SB:OnInitialize()
	...
	self:RegisterChatCommand("sbguild", "slashcommand_guild")
...
function SB:build_database()
	...
	-- In-memory table for provider-distributed guild blacklist entries.
	-- Rebuilt on every load so removals in provider updates take effect immediately.
	...
	for _, l in pairs(self.unprocessed_case_data) do
		if self:validate_provider(l) then
			...
			-- Guild data is processed regardless of enabled state,
			-- consistent with how the provider's realm scoping works.
			self:process_guild_data(l)
		end
	end
end
...
function SB:process_guild_data(l)
	-- Loads provider-distributed guild blacklist entries into the in-memory
...
function SB:check_unit_guild(unit_token)
	...
	-- (managed via /sbguild). Both are already realm-scoped.
	if not UnitIsPlayer(unit_token) then return end
	if UnitIsUnit("player", unit_token) then return end

	local conf = self:get_opts_db()
	if not conf.guild_blacklist_enabled then return end

	local guild = GetGuildInfo(unit_token)
	if not guild then return end

	-- Provider-distributed entries take priority, fall back to user entries.
	local entry = self.provider_guild_table[guild] or self.db.realm.guild_blacklist[guild]
	if not entry then return end

	-- Apply the same alert lockout period used for player alerts.
	local lockout_key = "guild:" .. guild
	local timeNow = GetServerTime()
	if guild_alert_lockout[lockout_key] then
		if timeNow < conf.alert_lockout_seconds + guild_alert_lockout[lockout_key] then
			return
		end
	end
	guild_alert_lockout[lockout_key] = timeNow

	self:raise_guild_alert(unit_token, guild, entry)
end

function SB:raise_guild_alert(unit_token, guild, entry)
	-- Fires a chat and/or sound alert for a guild blacklist hit.
	local conf = self:get_opts_db()
	local name = UnitName(unit_token) or "Unknown"
...
function SB:check_unit(unit_token, unit_guid, scan_context)
	...
	-- Guild blacklist check runs independently of GUID/name matching.
	-- Requires a unit_token so we can call GetGuildInfo.
	if unit_token then self:check_unit_guild(unit_token) end
	local conf = self:get_opts_db()
...
function SB:GROUP_ROSTER_UPDATE()
	...
	-- Guild check per unit token (check_unit skips guild when token is nil).
	for i = 1, n do
		if UnitExists(unit..i) then
			self:check_unit_guild(unit..i)
		end
	end
end
...
function SB:slashcommand_guild(input)
	-- /sbguild add <GuildName> | <Reason>
	-- /sbguild remove <GuildName>
	-- /sbguild list
	-- /sbguild on
	-- /sbguild off
	...
	if not input or input == "" then
		self:Print("Guild Blacklist commands:")
		print("  /sbguild add <Name> | <Reason>")
		print("  /sbguild remove <Name>")
		print("  /sbguild list")
		print("  /sbguild on / off")
		local state = self:get_opts_db().guild_blacklist_enabled and "|cff00ff00ENABLED|r" or "|cffff4444DISABLED|r"
		print("  Status: " .. state)
		return
	end
	...
	if cmd == "on" then
		self:get_opts_db().guild_blacklist_enabled = true
		self:Print("Guild blacklist |cff00ff00ENABLED|r.")
		return
	end

	if cmd == "off" then
		self:get_opts_db().guild_blacklist_enabled = false
		self:Print("Guild blacklist |cffff4444DISABLED|r.")
		return
	end

	if cmd == "list" then
		local count = 0
		-- Provider-distributed guilds
		...
		if count == 0 then
			self:Print("No guilds blacklisted.")
		else
			self:Print(count .. " guild(s) on blacklist.")
		end
		return
	end
	...
	if cmd == "add" then
		...
		if self.db.realm.guild_blacklist[guildName] then
			self:Print("|cffffcc00<" .. guildName .. ">|r is already blacklisted. Use /sbguild remove first to replace it.")
			return
		end
	...
	if cmd == "remove" then
		if not rest or rest == "" then
			self:Print("Usage: /sbguild remove <GuildName>")
			return
		end
		local guildName = trim(rest)
		if not self.db.realm.guild_blacklist[guildName] then
			self:Print("|cffffcc00<" .. guildName .. ">|r not found on blacklist.")
			return
		end
		self.db.realm.guild_blacklist[guildName] = nil
		-- Clear lockout so a re-add takes effect immediately.
		guild_alert_lockout["guild:" .. guildName] = nil
		self:Print("|cffff4444Removed:|r <" .. guildName .. "> from blacklist.")
		return
	end

	self:Print("Unknown command. Type /sbguild for usage.")
end
